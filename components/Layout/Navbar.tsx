...
const Navbar: React.FC = () => {
  ...
  const handleNavClick = (e: React.MouseEvent<HTMLAnchorElement>, href: string) => {
    ...
    // Handle section links with offset
    ...
    <>
      <header 
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 ease-in-out ${
          isScrolled ? 'bg-base-white/90 backdrop-blur-md py-4 shadow-sm' : 'bg-transparent py-8'
        }`}
      >
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 flex justify-between items-center">
          ...
          <nav className="hidden md:flex gap-8 flex-1">
            {navLinks.slice(0, 2).map((link) => (
              <a 
                ...
                className="font-sans text-xs uppercase tracking-widest text-charcoal hover:text-gold transition-colors duration-300 cursor-hover font-medium"
              >
                {link.name}
              </a>
            ))}
          ...
          {/* Logo Center */}
          <div className="flex-shrink-0 flex items-center justify-center flex-1 text-center">
            <a 
              href="#" 
              onClick={(e) => handleNavClick(e, '#')}
              className="flex flex-col items-center group cursor-hover"
            >
              {/* SVG Logo - Rostro Dorado (Updated) */}
              <div className={`h-6 md:h-9 w-auto transition-colors duration-300 ${isScrolled ? 'text-charcoal' : 'text-charcoal'}`}>
                <svg 
                  version="1.2" 
              ...
              <span className={`font-sans font-medium text-[10px] uppercase tracking-[0.4em] mt-1 transition-colors duration-500 ${isScrolled ? 'text-gold group-hover:text-charcoal' : 'text-charcoal group-hover:text-gold'}`}>
                Clinic
              </span>
            </a>
          </div>
          ...
          {/* Mobile Toggle */}
          <button 
            className="md:hidden text-charcoal cursor-hover z-50"
